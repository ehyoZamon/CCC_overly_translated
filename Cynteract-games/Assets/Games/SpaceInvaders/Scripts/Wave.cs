...
public class Wave : MonoBehaviour {
    ...
    IEnumerator CreateEnemyWave() //depending on chosed parameters generating enemies and defining their parameters
    {
        ...
        {
            GameObject newEnemy;
            if(SpaceInvaders.instance.Money >= 220)
            {
                newEnemy = Instantiate(enemy3, enemy3.transform.position, Quaternion.identity);
            }
            else if(SpaceInvaders.instance.Money >= 80)
            {
                newEnemy = Instantiate(enemy2, enemy2.transform.position, Quaternion.identity);
            }
            else
            {
            ...
            followComponent.speed = speed = speed + (0.004f * SpaceInvaders.instance.Money);
