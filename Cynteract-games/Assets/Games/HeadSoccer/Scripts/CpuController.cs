...
namespace Cynteract.SoccerGame
{
	public class CpuController : MonoBehaviour
	{
		...
		private Vector2 cpuFieldLimits = new Vector2(-2f, 8.5f);
		...
		void FixedUpdate()
		{
			if (canMove && !SoccerGameGameController.isGoalTransition && !SoccerGameGameController.gameIsFinished)
				playSoccer();
		}
		...
		void playSoccer()
		{
			...
			else if ((ball.transform.position.x > cpuFieldLimits.x && ball.transform.position.x < cpuFieldLimits.y))
			{
				...
				transform.position = new Vector3(Mathf.SmoothStep(transform.position.x, ball.transform.position.x + adjustingPosition+1, Time.deltaTime * moveSpeed),
