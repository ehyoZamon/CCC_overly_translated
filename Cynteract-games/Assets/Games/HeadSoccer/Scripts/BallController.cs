...
namespace Cynteract.SoccerGame
{
	public class BallController : MonoBehaviour
	{
		...
		void ManageBallShadow()
		{
			...
			ballShadow.transform.localScale = transform.position.y*0.3f* new Vector3(1.5f, 0.75f, 0.001f);
		}
		...
		void OnCollisionEnter(Collision other)
		{
			...
			if (goal)
			{
				if (goal.side==GameSide.Player)
				{
					StartCoroutine(gc.GetComponent<SoccerGameGameController>().ManageGoalEvent(GameSide.Player));
					ResetBallPosition(0);

				}
				else
				{
					StartCoroutine(gc.GetComponent<SoccerGameGameController>().ManageGoalEvent(GameSide.CPU));
					ResetBallPosition(1);
			...
			if (other.gameObject.tag == "Field")
			{
				...
				createHitGfx();
				//CheckHitGround();
				rigidbody.velocity = new Vector3(rigidbody.velocity.x, 10f);
			}
		...
		void CheckHitGround()
		{
			...
			if (SoccerGameGameController.gameIsFinished)
				return;

			if (transform.position.x < 0)
			{
				//ball has landed in the left field. we received a goal :(
				StartCoroutine(ResetBallPositionRoutine(1));
				//StartCoroutine(gc.GetComponent<SoccerGameGameController>().ManageGoalEvent(0));

			}
			else if (transform.position.x > 0)
			{
				//ball has landed in the right field. we scored a goal :)
				StartCoroutine(ResetBallPositionRoutine(0));
				//StartCoroutine(gc.GetComponent<SoccerGameGameController>().ManageGoalEvent(1));
			}
		...
		/// </summary>
		public IEnumerator ResetBallPositionRoutine(int _posIndex)
		{
			ResetBallPosition(_posIndex);
		...
		private void ResetBallPosition(int _posIndex)
		{
